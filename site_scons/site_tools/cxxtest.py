...
def multiget(dictlist, key, default=None):
    """Takes a list of dictionaries as its 1st argument.

    Checks if the key exists in each one and returns the 1st one it
    finds. If the key is found in no dictionaries, the default is
    returned.
    """
...
def envget(env, key, default=None):
    """Look in the env, then in os.environ.

    Otherwise same as multiget.
    """
...
def findCxxTestGen(env):
    """Locate the cxxtestgen script by checking environment, path and
    project."""
